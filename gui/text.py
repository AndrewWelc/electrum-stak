...
import electrum_stak as electrum
from electrum_stak.util import format_satoshis, set_verbosity
from electrum_stak.bitcoin import is_address, COIN, TYPE_ADDRESS
from electrum_stak import Wallet, WalletStorage
...
class ElectrumGui:
    ...
    def __init__(self, config, daemon, plugins):
        ...
        if not storage.file_exists():
            print("Wallet not found. try 'electrum-stak create'")
            exit()
    ...
    def network_dialog(self):
        ...
        if out:
            ...
            if out.get('server') or out.get('proxy'):
                proxy = electrum_stak.network.deserialize_proxy(out.get('proxy')) if out.get('proxy') else proxy_config
