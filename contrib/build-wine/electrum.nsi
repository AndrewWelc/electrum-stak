...
;Variables
  ...
  !define PRODUCT_NAME "Electrum-STAK"
  !define PRODUCT_WEB_SITE "https://lab.straks.co/straks/electrum-stak"
  !define PRODUCT_PUBLISHER "STRAKS"
...
;General
  ...
  ;Name and file
  Name "${PRODUCT_NAME}"
  OutFile "dist/electrum-stak-setup.exe"
  ...
  VIAddVersionKey LegalCopyright "2018 ${PRODUCT_PUBLISHER}"
...
;Interface Settings
  ...
  !define MUI_ICON "tmp\electrum-stak\icons\electrum.ico"
...
Section
  ...
  CreateShortCut "$DESKTOP\${PRODUCT_NAME}.lnk" "$INSTDIR\electrum-stak-${PRODUCT_VERSION}.exe" ""

  ;Create start-menu items
  DetailPrint "Creating start-menu items..."
  CreateDirectory "$SMPROGRAMS\${PRODUCT_NAME}"
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\Uninstall.lnk" "$INSTDIR\Uninstall.exe" "" "$INSTDIR\Uninstall.exe" 0
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\${PRODUCT_NAME}.lnk" "$INSTDIR\electrum-stak-${PRODUCT_VERSION}.exe" "" "$INSTDIR\electrum-stak-${PRODUCT_VERSION}.exe" 0
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\${PRODUCT_NAME} Testnet.lnk" "$INSTDIR\electrum-stak-${PRODUCT_VERSION}.exe" "--testnet" "$INSTDIR\electrum-stak-${PRODUCT_VERSION}.exe" 0


  ;Links straks: URI's to Electrum
  WriteRegStr HKCU "Software\Classes\straks" "" "URL:straks Protocol"
  WriteRegStr HKCU "Software\Classes\straks" "URL Protocol" ""
  WriteRegStr HKCU "Software\Classes\straks" "DefaultIcon" "$\"$INSTDIR\electrum.ico, 0$\""
  WriteRegStr HKCU "Software\Classes\straks\shell\open\command" "" "$\"$INSTDIR\electrum-stak-${PRODUCT_VERSION}.exe$\" $\"%1$\""
...
Section "Uninstall"
  ...
  DeleteRegKey HKCU "Software\Classes\straks"
