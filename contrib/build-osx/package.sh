...
if [ -z "$1" ]; then
    echo "Usage: $0 Electrum-STAK.app"
    exit -127
fi
...
if ! which ${genisoimage} > /dev/null 2>&1; then
	mkdir -p /tmp/electrum-stak-macos
	cd /tmp/electrum-stak-macos
...
rm -rf /tmp/electrum-stak-macos/image > /dev/null 2>&1
mkdir /tmp/electrum-stak-macos/image/
cp -r $1 /tmp/electrum-stak-macos/image/
...
dmg dmg Electrum-STAK_uncompressed.dmg electrum-stak-$VERSION.dmg || fail "Unable to create compressed dmg"
rm Electrum-STAK_uncompressed.dmg

echo "Done."
md5sum electrum-stak-$VERSION.dmg
